...
class A3C(agent.AttributeSavingMixin, agent.AsyncAgent):
    ...
    def update(self, statevar):
        ...
        if self.recurrent:
            ...
            # EDIT: Extra Squeeze for shape match
            batch_v = batch_v # .squeeze(-1)

        batch_action = torch.stack(
            [self.past_action[i] for i in range(self.t_start, self.t)]
        )

        # EDIT: Extra Flatten for shape match
        batch_log_prob = batch_distrib.log_prob(batch_action) # .flatten()
        
        # EDIT: Extra Flatten for shape match
        batch_entropy = batch_distrib.entropy() # .flatten()
        ...
        # EDIT: Extra Flatten for shape match
        batch_return = batch_return # .flatten()

        batch_adv = batch_return - batch_v.detach().squeeze(-1)

        # print(batch_log_prob.shape, batch_entropy.shape, batch_adv.shape)
        # print(batch_v.shape, batch_return.shape)

        assert batch_log_prob.shape == (n, 1), f'{batch_log_prob.shape}'
        assert batch_adv.shape == (n, 1), f'{batch_adv.shape}'
        assert batch_entropy.shape == (n, 1), f'{batch_entropy.shape}'
        pi_loss = torch.sum(
            -batch_adv * batch_log_prob - self.beta * batch_entropy, dim=0
        )
        assert batch_v.shape == (n, 1, 1)
        assert batch_return.shape == (n, 1)
    ...
    def _observe_train(self, obs, reward, done, reset):
        # print("Observe Train Obs Shape:", obs.shape)
    ...
    def _act_eval(self, obs):
        ...
        with torch.no_grad(), pfrl.utils.evaluating(self.model):
            ...
            if self.act_deterministically:
                ...
                # return mode_of_distribution(pout).cpu().numpy()
            else:
                return pout.sample().cpu().numpy()[0]
                # return pout.sample().cpu().numpy()
